...
Do not speculatively refactor unshipped subsystems (storage/retention, room eviction) ahead of their milestone. `db.persistState`, `createWriteQueue`, and `db.getSheet` are retained-but-uncalled **M5/M10 seams** — they are not dead code and must not be deleted.
...
5. Ask for skeptical review before committing meaningful changes.
